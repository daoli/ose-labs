...
boot_alloc(uint32_t n)
{
	...
	char *result;
	unsigned int avail_npages, request_npages;
	...
	// hmm, should we do this here?
	if (npages > 1024 * 64) {
		panic("boot_alloc: memory is more than 256MB.");
	}

	nbytes_used = (uintptr_t) nextfree - KERNBASE;
	avail_npages = npages - ROUNDUP(nbytes_used, PGSIZE) / PGSIZE;
	request_npages = n / PGSIZE + (n % PGSIZE != 0);

	// Allocate a chunk large enough to hold 'n' bytes, then update
	// nextfree.  Make sure nextfree is kept aligned
	// to a multiple of PGSIZE.
	result = nextfree;
	if (request_npages < avail_npages) {
		nextfree += PGSIZE * request_npages;
	} else if (request_npages == avail_npages) {
	        nextfree = (char *) 0xFFFFFFFF;
	} else {
		panic("boot_alloc: memory used up!");
	}
...
mem_init(void)
{
	...
	if (page_pse) {
		...
		boot_map_region(kern_pgdir, KERNBASE,
				ROUNDUP(0xFFFFFFFF-KERNBASE, PGSIZE), 0x0, PTE_W);
	}
...
page_init(void)
{
	...
	for (; i < npages_basemem + npages_io + npages_kern; i++) {
		pages[i].pp_ref = 1;
...
pgdir_walk(pde_t *pgdir, const void *va, int create)
{
	...
	// visit page diretory, create corresponding page table if requested.
	pdep = &pgdir[PDX(va)];
	if (!(*pdep & PTE_P)) {
		if (!create) {
			return NULL;
		...
		pp->pp_ref++;
		*pdep = page2pa(pp) | PTE_U | PTE_W | PTE_P;
	}
	// go to second level page table
	pdep = KADDR(PTE_ADDR(*pdep));
...
boot_map_region(pde_t *pgdir, uintptr_t va, size_t size, physaddr_t pa, int perm)
{
	...
	for (i = 0; i < size; i += PGSIZE) {
		if (!(ptep = pgdir_walk(pgdir, (void *) (va + i), 1))) {
...
page_insert(pde_t *pgdir, struct Page *pp, void *va, int perm)
{
	...
	// we should increase the reference count before calling page_remove,
	// because if we re-insert the same pp and its reference count is 1,
        // reversing the order will cause pp to be moved to free list.
	pp->pp_ref++;
	if (*ptep & PTE_P) {
